...
// CSVFILE resides in the directory of the current user
var CSVFILE = "/Users/kchou/Code/mastering-Go/ch04/phoneBook/csv.data"
...
func insert(pS *Entry) error {
	...
	_, ok := index[(*pS).Tel]
	if ok {
		return fmt.Errorf("Entry already exists: %s", pS.Tel)
	}
...
func setCSVFILE() error {
	...
	mode := fileInfo.Mode()
	if !mode.IsRegular() {
		return fmt.Errorf("%s is not a regular file", CSVFILE)
	}
...
func main() {
	arguments := os.Args
	if len(arguments) == 1 {
		fmt.Println("Usage: insert|delete|search|list|listReverse <arguments>")
		return
	}
	...
	case "insert":
		...
		if !matchTel(t) {
			fmt.Println("Invalid telephone number:", t)
			return
		}
